          ...
          <div class="div_p">
            <div class="order_goods" v-for="(item,index) in scope.row.orderGoodsList" :key="index">
              ...
              <div style="width:120px;text-align: center; padding: 20px; flex-shrink: 0;">
                ...
                <el-button class="text mar_left" type="primary" size="mini" v-if="scope.row.payStatus ==2 && scope.row.orderStatus  == 5 &&(item.hadComment== 1 || item.hadComment== 3)" @click="goEvaluation(item,3)">查看评论</el-button>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    goEvaluation(data, index) {
      if (index == 1) {
        ...
        this.$router.push({ name: 'evaluationDetial', query: { commentId: data.commentId, orderId: data.orderId } })
      }
    },
    ...
    async getAllorder() {
      ...
      try {
        ...
        this.$emit('getCommentOrderNum', this.total)
      } catch (e) {
        this.$message.error('未知错误!!!')
      }
    },
