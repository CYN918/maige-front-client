    ...
    <div class="box-2" id="box-2">
      ...
      <ul class="list">
        <li class="item item_bc1" v-if="isShowSku">
         ...
         <li class="item item_bc1 pintuan" style="padding-top:0px" v-else >
            <div class="fightGroup">
              ...
              <span class="num">已团{{num}}件</span>
            </div>
            <div style=" width: 100%;display: flex;"> 
              <span class="t1">拼团价</span>
              <span class="t1-colon">：</span>
        ...
        <li class="item middle">
          ...
          <span class="t2 storage">{{price.quantity ? `库存${price.quantity}件` : '库存不足'}}</span>
        </li>
      ...
      <!--拼团活动-->
      <div>
        <FightGroup v-if="!isShowSku" :number="number" :skuList="skuList" :data="data"></FightGroup>
      </div>
    </div>
    <div class="box-3">
      ...
      <div class="right_div1">
        <div class="product-1">
          <a :href="price.jdGoodsUrl" class="img-wrap" @mouseenter="moveTip" @mouseleave="leaveTip">
          ...
          <p class="text">对比价：{{(price.jdPrice ==0 || price.jdPrice =='')?'暂无':price.jdPrice+'元'}}</p>
        </div>
        <div class="product-1">
          <a
          ...
          <p class="text">对比价：{{(price.tmallPrice==price.tmallPrice =='')?"暂无":price.tmallPrice+'元'}}</p>
        </div>
      </div>
    </div>
  </div>
...
import { addShoppingCart, obtainSku ,getGroupTime} from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    ShowPrice(data, index, index2, item) {
      ...
      if (this.obtainSku == "") {
        ...
        const skuId = this.skuidList[index2].skuId;
        this.findData(skuId);
        this.storeId = skuId;
      }
    },
    ...
    findData(value) {
      ...
      if (this.skuidList === "") {
        ...
        for (let i = 0; i < this.skuList.length; i++) {
          console.log(this.skuList[i].skuId, value);
          if (this.skuList[i].skuId === value) {
            const obj = {
              quantity: this.skuidList[i].enableQuantity,
              goodsPrice: this.skuidList[i].salePrice,
              skuId: this.skuidList[i].skuId,
              thumbnailImgUrl: this.skuidList[i].thumbnailImgUrl
            };
            this.price = obj;
            this.ImageArr(arr[i]);
            console.log(arr[i]);
    ...
    calcMaxLeft() {
      ...
      const maxLeft = ulWidth - containerWidth;
      return maxLeft;
    },
    moveToPrev() {
  ...
  // 活动页传的商品skuid
  watch: {
    skuList(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.skuidList = newVal;
        const arr = this.data.skuDetailList || [];
        for (let i = 0; i < arr.length; i++) {
          // 活动页面进入的
          const skuId = arr[i].skuId;
          for (let j = 0; j < this.skuidList.length; j++) {
            if (this.skuidList[j].skuId === skuId) {
              this.price = arr[i];
              this.price.goodsPrice = this.skuidList[j].salePrice;
              this.price.quantity = this.skuidList[j].enableQuantity;
              this.ImageArr(arr[i]);
              return;
            } else if (
              i === arr.length - 1 &&
              this.skuidList[i].skuId !== skuId
            ) {
              this.price = {};
              return false;
            }
