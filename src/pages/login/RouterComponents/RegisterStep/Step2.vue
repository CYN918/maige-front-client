...
<script>
import { registerByMessage, goLogin } from '@/api/index'
import { handleCrypt, setCookie } from '@/assets/utils/utils'
...
export default {
  ...
  methods: {
    ...
    async setSuccess() {
      ...
      this.captcha = sessionStorage.getItem('ReCaptcha')
      ...
      const res = await registerByMessage(obj)
      if (res.code === 200) {
        // 自动登录
        ...
        const result = await goLogin(formData)
        if (result.code === 200) {
          setCookie('userData', JSON.stringify(result.data), 1)
        }
