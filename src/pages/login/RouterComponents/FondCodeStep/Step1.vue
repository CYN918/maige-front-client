  ...
  <div>
    <form action1="">
      ...
      <div class="row">
        ...
        <el-button v-if="!isSendVC" class="verification" type="text" @click="getRegisterCaptcha">
          <span>获取验证码</span>
        </el-button>
        <el-button v-else class="verification" type="text">
...
<script>
import { sendRetrievePasswordCaptchaMessage, checkRegisterCaptcha, getSmsSendTime } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    async getRegisterCaptcha() {
      ...
      try {
        const res = await sendRetrievePasswordCaptchaMessage(this.phoneNum);
        if (res.code === 200) {
          sessionStorage.setItem('findPhoneNum', this.phoneNum)
          this.isSendVC = true;
          this.phoneNumTip = ''
          this.showPhoneNumTip = false
        } else {
    ...
    async checkNextStep() {
      ...
      if (res.code === 200) { // 暂时为了通过，需要改为200
        ...
        sessionStorage.setItem('findCaptcha', this.captcha)
        this.$router.push('/fstep2')
      } else {
        this.showVerificationTip = true
