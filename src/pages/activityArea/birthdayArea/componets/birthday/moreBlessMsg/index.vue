<template>
  <div>
    <div class="headers" v-if="receivGiftContainer.length>0">
      <div class="text">
        <ul class="leaveMessageWrap" v-for="(item,indexs) in receivGiftContainer" :key="indexs">
            ...
            <div class="leaveMessageClildrens"    v-for="(children,sunindex) in item.childComment" :key="sunindex">
              ...
              <div v-if="children.childReply.length>0" >
                ...
                <div class="blessContainer" v-if="isshowPublish&&sunNum===index&&sunNumtype===sunindex">
                  ...
                  <div class="submit">
                    <span  class="send-bless-msg-btn"  @click="sunpub(children,items,index)">回复</span>
...
export default {
  ...
  computed:{
    ...mapState(['birthdayList','activityId','userName','gifterUserId','userId'])
  },
  methods: {
    //评论
    async pub(item) {
    ...
    //二级回复
    async sunpub(children,item,index){
      console.log(item)
      if(this.sunContent==='') return
      const obj = {
