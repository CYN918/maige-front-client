<template>
  <div id="all_order_tab">
    ...
    >
        ...
        <template slot-scope="scope">
          <div class="order_no">
            ...
            <div class="button">
              <el-button
                ...
                size="mini"
                v-show="true"
                @click.stop="handleCheck(scope.row,'/orderdetails')"
              >查看详情</el-button>
              <el-button
                class="text"
                type="primary"
                size="mini"
                v-if="scope.row.payStatus  == 0 && scope.row.orderStatus  != 2"
              ...
              <!-- <el-button
                ...
                type="primary"
                size="mini"
                v-if=" scope.row.payStatus ==2 && scope.row.orderStatus  == 5 && (scope.row.hadComment==0 || scope.row.hadComment==2) && scope.row.afterSaleStatus == 0"
                @click="complefunc(scope.row)"
              >申请售后</el-button> -->
              <el-button
                class="text"
                type="primary"
              ...
              <el-button
                ...
                type="primary"
                size="mini"
                v-if="scope.row.payStatus ==2 && scope.row.orderStatus  != 2 && scope.row.shippingStatus != 0 "
              ...
              <el-button
                ...
                type="primary"
                size="mini"
                v-if="scope.row.payStatus ==2 &&(scope.row.shippingStatus== 0 || scope.row.shippingStatus== 4) && scope.row.orderStatus != 2"
                @click="handleState(scope.row.orderId)"
              >提醒发货</el-button>
              <el-button
                class="text"
                type="primary"
                size="mini"
                v-if="scope.row.payStatus ==2 && scope.row.orderStatus  != 2 &&(scope.row.shippingStatus== 1 || scope.row.shippingStatus== 4)"
                @click="suergoods(scope.row.orderId)"
              >确认收货</el-button>
              <el-button
                class="text"
                type="primary"
          ...
          <div class="div_p">
            <div class="order_goods" v-for="(item,index) in scope.row.orderGoodsList" :key="index">
              ...
              <span style="width:110px;text-align: center; padding: 10px; flex-shrink: 0;">{{item.shippingName}}</span>
              <div style="width:120px;text-align: center; padding: 20px; flex-shrink: 0;">
                <span v-if="!(scope.row.payStatus ==2 && ( scope.row.orderStatus  == 5 ||  scope.row.orderStatus  == 4) && item.afterSaleStatus == 0)">{{scope.row.statusMsg}}</span>
                <el-button
                  class="text"
                  type="primary"
                  size="mini"
                  v-if="scope.row.payStatus ==2 && scope.row.orderStatus  == 5 && item.hadComment== 1 "
                  @click="goEvaluation(item,2)"
                >追评</el-button>
                <el-button
                  class="text"
                  type="primary"
                  size="mini"
                  v-if="scope.row.payStatus ==2 && scope.row.orderStatus  == 5 && item.hadComment== 0 "
                  @click="goEvaluation(item,1)"
                >去评价</el-button>
                <span v-if="scope.row.payStatus !=2 && ( scope.row.orderStatus  != 5 ||  scope.row.orderStatus  != 4) && item.afterSaleStatus != 0">{{scope.row.statusMsg}}</span>
              </div>
            </div>
          </div>
          <package v-if="scope.row.showDetail" :logdata="scope.row"></package>
        </template>
      </el-table-column>
      ...
      <el-table-column  :resizable="false" width="120" label="状态/操作" align="center"></el-table-column>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible1" width="30%">
      ...
      <span slot="footer" class="dialog-footer txt_center">
        <el-button
          ...
          style="background:#F56C6C;width:96px;height:36px;line-height: 10px;"
          type="danger"
        >取 消</el-button>
        <el-button
          @click="payOrderIds"
          style="background:#4A99FA;width:96px;height:36px;line-height: 10px;"
          type="primary"
        >确 定</el-button>
      </span>
    </el-dialog>
    ...
    <!-- 发货弹窗 -->
    <!-- 支付状态弹窗 -->
    <el-dialog title="支付提示" :visible.sync="dialogVisible" width="30%">
    ...
    <el-dialog title="确认收货" :visible.sync="centerDialogVisible" width="30%">
      <span>是否确认收货</span>
      <span slot="footer" class="dialog-footer">
        <el-button type="danger" @click="centerDialogVisible = false" size="small">取 消</el-button>
        <el-button type="primary" @click="suergoods(false)" size="small">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      suergoodid: "",
      centerDialogVisible: false, //收货弹窗
  ...
  mounted() {
    this.getAllorder();
    this.action =
      BASE_URL + "/api/open/c/b2b2c/orderservice/order/addressImport.json";
  },
  methods: {
    // 支付遇到问题
    handleMeetProblem() {
    ...
    async deliverGoodsfunc(data) {
      ...
      res.data.orderGoodsList.forEach(item => {
        item.statusMsg = res.data.statusMsg;
      });
    ...
    goEvaluation(data, index) {
      if (index == 1) {
        ...
        this.$router.push({
          name: "review",
          query: { commentId: data.commentId, orderId: data.orderId }
        });
      }
    },
