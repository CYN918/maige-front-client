<template>
        ...
        <template slot-scope="scope">
          <div class="order_no">
            ...
            <div class="button">
              ...
              <el-button class="text" type="primary" size="mini" v-if="scope.row.payStatus ==2 && scope.row.orderStatus  != 2 && scope.row.shippingStatus != 0 " @click="Logistics(scope.row)">物流信息</el-button>
              ...
              <el-button class="text" type="primary" size="mini" v-if="scope.row.orderType== 1 && scope.row.reciveAddress== null && scope.row.orderStatus  != 2" @click='deliverGoodsfunc(scope.row)'>填写地址</el-button>
            </div>
          </div>
          <div class="div_p">
            <div class="order_goods" v-for="(item,index) in scope.row.orderGoodsList" :key="index">
              ...
              <div style="width:120px;text-align: center; padding: 20px; flex-shrink: 0;">
                ...
                <el-button class="text" type="primary" size="mini" style='margin-left: 0;margin-top: 2px;' v-if="scope.row.payStatus ==2 && ( scope.row.orderStatus  == 5 ||  scope.row.orderStatus  == 4) && item.afterSaleStatus == 0"   @click="complefunc(scope.row)">申请售后</el-button>
              </div>
            </div>
          </div>
  ...
  <span slot="footer" class="dialog-footer txt_center">
    ...
    <!-- 支付状态弹窗 -->
    <shipped-dialog :dialog='showafter' @closediv='changedia'  :passdata="passdata"></shipped-dialog>
...
export default {
  ...
  data() {
    return {
      suergoodid:'',
      passdata:{},
      centerDialogVisible:false, //确认收货弹窗
      showafter:false, //售后弹窗
  ...
  methods: {
    ...
    async payOrderIds() {
      ...
      const linkUrl = 'payment.html?redirect=' + url + '&paymode=' + this.paymode + '&orderNo=' + this.orderSn + '#' + page
      window.open(linkUrl)
...
<style lang="stylus" scoped>
  ...
  /deep/ .spicallist .el-table__body-wrapper {
