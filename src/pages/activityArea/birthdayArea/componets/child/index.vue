<template>
  <div>
    ...
    <div class="birthday-card">
      <h3 class="birthday-card-title">生子</h3>
        ...
        <li class="birthday-card-01">
          <div class="user-wrap">
            <div class="avantar-wrap">
              ...
              <img class="birthday-avantar" src='@/assets/images/birthdayArea/child.png' alt="生子" />
            </div>
            <div class="birthday-name-wrap">
              <span class="birthday-name">{{childPersonal.userName}}</span>
              <img
                class="icon-button"
      ...
      <div class="pop-up">
        ...
        <ul class="pop-up-content clearfix">
          ...
          >
            ...
            <img class="select-avantar-pic" src='@/assets/images/birthdayArea/child.png' alt />
...
import {getGift ,getaddBlessing,getReceivGift,userInfo}  from '@/api/index'
export default {
  data(){
    return{
      addBlessing:'',
  ...
  computed:{
    ...mapState(['childPersonal','childUserId','gifterUserId'])
  },
  methods:{
    //选择人物
    clilkpersonal(index) {
      this.num = index;
      this.$store.commit('childUserId',this.userInfo[this.num].userId)
    },
    //控制弹窗
    isshowMsg() {
      this.ishow = true;
    ...
    async gilr() {
      ...
      const obj = {
        ...
        happyEventType:3,   //活动类型
        userId:this.childUserId ,  //评论者id
        presentType:1     //祝福类型
      }
  ...
  async  showMsg() {
      this.ishow = false;
      this.$store.commit('childPersonal',this.userInfo[this.num])
      const  obj = {
    ...
    pickGift(){
      ...
      this.$router.push('/pickGiftchild')
    },
    //打赏红包
    redEnvelope(){
      if(!this.childUserId){
      ...
      this.$router.push('/redEnvelopechild')
    },
    //祝福留言
    blessingMessage(){
      if(!this.childUserId){
  ...
  async created(){
    ...
    function parseUrl(){
      ...
      this.activityTypeId = v['activityTypeId'].split('#')[0]
      const obj = {
        activityId,
        happyEventType:3,
        pageIndex:1,
