<template>
  <div>
    ...
    <div class="birthday-card">
      <h3 class="birthday-card-title">升职</h3>
        ...
        <li class="birthday-card-01">
          <div class="user-wrap">
            <div class="avantar-wrap">
              ...
              <img class="birthday-avantar" :src='promotionPersonal.sex===2?birthdayPhonto[1].img:birthdayPhonto[0].img' alt="结婚" />
            </div>
            <div class="birthday-name-wrap">
              <span class="birthday-name">{{promotionPersonal.userName}}</span>
              <img
                class="icon-button"
      ...
      <div class="pop-up">
        ...
        <ul class="pop-up-content clearfix">
          ...
          >
            ...
            <img class="select-avantar-pic" :src='promotionPersonal.sex===2?birthdayPhonto[1].img:birthdayPhonto[0].img' alt />
            <p class="selected-avantar-name">{{item.userName}}</p>
          </li>
        </ul>
         <el-pagination
            class="pagination"
...
import {getGift ,getaddBlessing,userInfo}  from '@/api/index'
export default {
  data(){
    return{
      addBlessing:'',
      ...
      userInfo:'',
      birthdayPhonto:[
        {img:require('@/assets/images/birthdayArea/shengzhi.png')},
        {img:require('@/assets/images/birthdayArea/shengzhi2.png')}
      ],
      activityId:''
    }
  },
  ...
  computed:{
    ...mapState(['promotionPersonal','promotionUserId','gifterUserId'])
  },
  methods:{
    //选择人物
    clilkpersonal(index) {
      this.num = index;
      this.$store.commit('promotionUserId',this.userInfo[this.num].userId)
    },
    //控制弹窗
    isshowMsg() {
      this.ishow = true;
    ...
    async gilr() {
      ...
      const obj = {
        ...
        userId:this.promotionUserId ,  //评论者id
        presentType:1     //祝福类型
      }
  ...
  async  showMsg() {
      this.ishow = false;
      this.$store.commit('promotionPersonal',this.userInfo[this.num])
       const  obj = {
    ...
    pickGift(){
      ...
      this.$router.push('/pickGiftpromotion')
    },
    //打赏红包
    redEnvelope(){
      if(!this.promotionUserId){
      ...
      this.$router.push('/redEnvelopepromotion')
    },
    //祝福留言
    blessingMessage(){
      if(!this.promotionUserId){
