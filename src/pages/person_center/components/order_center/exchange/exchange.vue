<template>
  <div id="all_order_tab">
    <el-table
      ...
      <el-table-column :resizable="false" label="商品详情" align="center" width='255'>
        <template slot-scope="scope">
          <div class="order_no">
            ...
            <div class="button">
              ...
              <el-button class="text" type="primary" size="mini" v-if="scope.row.payStatus ==2 && scope.row.orderStatus  == 5 || scope.row.payStatus ==2 && scope.row.orderStatus  == 5 && (scope.row.hadComment==0 || scope.row.hadComment==2)">申请售后</el-button>
              ...
              <el-button class="text" type="primary" size="mini" v-if="scope.row.payStatus ==2 && scope.row.orderStatus  != 2 &&(scope.row.shippingStatus== 1 || scope.row.shippingStatus== 4)" @click="Logistics(scope.row)">包裹/售后</el-button>
              <el-button class="text" type="primary" size="mini" v-if="scope.row.payStatus ==2 &&(scope.row.shippingStatus== 0 || scope.row.shippingStatus== 4) && scope.row.orderStatus != 2" @click="handleState(scope.row.orderId)">提醒发货</el-button>
              <el-button class="text" type="primary" size="mini" v-if="scope.row.payStatus ==2 && scope.row.orderStatus  != 2 &&(scope.row.shippingStatus== 1 || scope.row.shippingStatus== 4)" @click='suergoods(scope.row.orderId)'>确认收货</el-button>
              ...
              <el-button class="text" type="primary" size="mini" v-if="scope.row.orderType== 1 && scope.row.reciveAddress== null" @click='deliverGoodsfunc(scope.row)'>填写地址</el-button>
            </div>
          </div>
          <div class="div_p">
            <div class="order_goods" v-for="(item,index) in scope.row.orderGoodsList" :key="index">
              <div style="width: 255px;height: 60px; padding-left: 20px; flex-shrink: 0; display:flex; align-items: center;">
              ...
              <span style="width:100px;text-align: center;padding: 10px; flex-shrink: 0;">{{item.goodsAttr}}</span>
              <span style="width:80px;text-align: center;padding: 10px; flex-shrink: 0;">{{item.goodsPrice}}</span>
              <span style="width:80px;text-align: center;padding: 10px; flex-shrink: 0;">{{item.quantity}}</span>
              <span style="width:80px;text-align: center;padding: 10px; flex-shrink: 0;">{{item.specialPrice}}</span>
              <span style="width:80px;text-align: center; padding: 10px;flex-shrink: 0;">{{item.goodsAmount}}</span>
              <span style="width:80px;text-align: center; padding: 10px; flex-shrink: 0;">{{scope.row.payName}}</span>
              <span style="width:80px;text-align: center; padding: 10px; flex-shrink: 0;">{{scope.row.shippingName}}</span>
              <div style="width:100px;text-align: center; padding: 20px; flex-shrink: 0;">
      ...
      <el-table-column  :resizable="false" width="120" label="规格" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="商城价" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="数量" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="结算价" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="合计" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="支付方式" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="物流信息" align="center"></el-table-column>
      <el-table-column  :resizable="false" width="100" label="状态/操作" align="center">
      </el-table-column>
  ...
  methods: {
    ...
    async handleState(id) {
      ...
      try {
        ...
        throttleTipPop(this, 'error', '未知错误')
      }
    },
    // 取消订单
