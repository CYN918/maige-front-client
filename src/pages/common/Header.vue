<template>
  <div class="wrapper" v-show="componentStatuHeader&&componentStatuHeader.componentContents[0].contentStatus==0">
    <div class="header">
      <div class="logo">
        <a href="index.html">
    ...
    <div style="width: 1200px; margin: 0 auto;">
      <ul class="list clearfix">
        <li class="_item"><a href="product_list.html" style="color: #fff">全部分类</a></li>
        <li class="_item" v-if="content.jdProduct"><a href="product_list.html?goodsType=1" :style="{color:jdColor}">京东商品</a></li>
        <li class="_item"  v-if="content.neteaseProduct"><a href="product_list.html?goodsType=3" :style="{color:neteaseProductColor}" >苏宁商品</a></li>
        <li class="_item" v-for="item in sourceList||[]">
          <a :href=item.sourceLink style="color: black">
            {{item.sourceName}}
          </a>
        </li>
      </ul>
  ...
  export default {
    ...
    data() {
      return {
        ...
        content:'',
        tableData:'',
        componentStatuHeader:'',
        sourceList:'', //来源分类
        goodsType:"",
        url:'',
        jdColor:'black',
        neteaseProductColor:'black'

      };
    },
    created(){
      this.url = addressUrl()
     if(this.url&&this.url['goodsType']){
       this.goodsType = this.url['goodsType']
       this.jdColor = this.goodsType&&this.goodsType=='1'? '#25a4bb': 'black'
       this.neteaseProductColor = this.goodsType&&this.goodsType=='3'? '#25a4bb': 'black'
     }
    },
    mounted() {
       this.componentStatuHeader = JSON.parse(window.localStorage.getItem('headerSearch'))
      this.title = this.componentStatuHeader.componentContents[0].title||''
      this.content = JSON.parse(this.componentStatuHeader.componentContents[0].content||'')
      console.log(this.content)
      this.tableData = this.content.tableData.sort(this.compare('sort'))
      this.sourceList = this.content.sourceList.sort(this.compare('sortOrder'))
    ...
    methods: {
      ...
      handleTipsWord() {
        ...
        if (e.target.nodeName.toLocaleLowerCase() === 'span') {
          ...
          if(this.goodsType){
            window.location.href =`product_list.html?goodsName${this.text}&goodsType=${this.goodsType}`
          }else{
      ...
      handleConfirm() {
        ...
        if(this.goodsType){
          if(!this.text){
            window.location.href = `product_list.html?goodsType=${this.goodsType}`;
          }else{
            window.location.href = `product_list.html?goodsName=${this.text}&goodsType=${this.goodsType}`;
          }
        }else{
